...
"""
function solve!(K::AbstractMatrix, b::AbstractVector, cache::NewtonCache)
...
Solve the nonlinear equation system r(x)=0 using the newton method. 
...
function newtonsolve!(x::AbstractVector, drdx::AbstractMatrix, rf!, cache::NewtonCache = NewtonCache(x,rf!); tol=1.e-6, max_iter=100)
    ...
    for _ = 1:max_iter
        ...
        solve!(DiffResults.jacobian(diffresult), diffresult.value, cache)
        x .-= diffresult.value
    end
    # No convergence
    return false
