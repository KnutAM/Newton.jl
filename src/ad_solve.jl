...
const NonDualInput = Union{<:AbstractFloat, <:AbstractArray{TD} where {TD<:AbstractFloat}}
 ...
 """
    ad_newtonsolve(rf, x0, rf_dual_args; kwargs...)
...
**Implementation:** Uses the adjoint, i.e. `dr/dyᵢ = 0 = ∂r/∂x ∂x/∂yᵢ + ∂r/∂yᵢ ⇒ ∂x/∂yᵢ = -[∂r/∂x]⁻¹ ⋆ ∂r/∂yᵢ`

    ad_newtonsolve(rf, x0, rf_args; kwargs...)
...
function _ad_newtonsolve(rf::F, x0::NonDualInput, rf_dual_args::DualInput...; kwargs...) where F
    ...
    dual_results = ntuple(i -> seed_dual_result(rf, x, Val(i), rf_dual_args[i], rf_args, drdx_inv), length(rf_dual_args))
    return sum(dual_results), converged
end

function _ad_newtonsolve(rf::F, x0::NonDualInput, rf_args::NonDualInput...; kwargs...) where F
    rf_singlearg(x) = rf(x, rf_args...)
    x, _, converged = newtonsolve(rf_singlearg, x0)
    return x, converged
end
...
function seed_dual_result(rf, x, ::Val{I}, dual_arg, rf_args, drdx_inv) where I
    ...
    return insert_value(x_dual_yᵢ, x) # Insert the correct value from `x` into the `x_dual_yᵢ` 
end
